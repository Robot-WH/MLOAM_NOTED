...
public:
    ...
    bool Evaluate(double const *const *param, double *residuals, double **jacobians) const
    {
        ...
        residual.bottomRows<3>() = 2 * (rot_.inverse() * Q).coeffs().head<3>(); //  旋转残差 ,  四元数转换为李代数   
        ...
        {   
            if (jacobians[0])
            {   //  关于李群的jacobian  但由于参数化的 原因   这里实际上还是求解关于李代数的jacobian  
                ...
                // TODO: the jacobian seem to be wrong, please fix   有误 ？？？？     求解关于旋转李代数的jacobian  
...
private:
    const Eigen::Vector3d pos_;   // 先验平移
    const Eigen::Quaterniond rot_;   // 先验旋转  
