...
{
    assert(pose_laser.size() == NUM_OF_LASER);   // 检查数据是否一致
    bool b_check = true;
    // 检查数据  
    for (size_t i = 0; i < pose_laser.size(); i++) 
        b_check = (b_check && checkScrewMotion(pose_laser[IDX_REF], pose_laser[i]));    // 检查转动是否充分  
    // 如果数据合格   
    if (b_check)
    {   
