...
class Estimator
{
  public:
    ...
    // 滑动窗口 特征 
    std::queue<std::pair<double, std::vector<cloudFeature> > > feature_buf_;   // 保存每一时刻的多激光数据    <时间t, <每一个激光的数据>>
    ...
    double **para_pose_{};       // 滑动窗口优化中，参与优化的参考激光雷达pose状态   一共有  OPT_WINDOW_SIZE + 1 个pose状态  
    double **para_ex_pose_{};     // 滑动窗口优化中， 参与优化的多激光外参状态 
    double *para_td_{};
