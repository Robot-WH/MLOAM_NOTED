...
/**
 *  @brief 与上一帧的线特征进行匹配  
 *  @details 当前帧的点去上一帧找到两个最近点组成直线  
 *  @param kdtree_corner_from_scan KD树
 *  @param cloud_scan 上一帧的scan
 *  @param cloud_data 当前帧scan 
 *  @param pose_local 相对位姿
 *  @param[out] features 匹配的结果 
 */
template <typename PointType>
                                         ...
                                         std::vector<PointPlaneFeature> &features)
{
    ...
    for (size_t i = 0; i < cloud_data.points.size(); i++)
    {
        ...
        TransformToStart(cloud_data.points[i], point_sel, pose_local, false, SCAN_PERIOD);     // 根据时间戳转换到 start
        ...
        if (point_search_sqdis[0] < DISTANCE_SQ_THRESHOLD)
        {
            ...
            // 下面两个 循环  目的是 在KDTREE搜索出来的上一帧最近点 的上下scan中各找到一个距离该点最近的点  组成直线
            // search in the direction of increasing scan line   遍历该点之后的全部点   
            for (int j = closest_point_ind + 1; j < (int)cloud_scan.points.size(); j++)
            {
                // if in the same scan line, continue
                if (int(cloud_scan.points[j].intensity) <= closest_point_scan_id)    // scan_id应该是++的  
                    continue;
                // if not in nearby scans, end the loop
                if (int(cloud_scan.points[j].intensity) > (closest_point_scan_id + NEARBY_SCAN))    //  NEARBY_SCAN  2.5 
                    break;
                // 计算距离  找到距离最小的
                                       ...
                                       std::vector<PointPlaneFeature> &features)
{
    ...
    for (size_t i = 0; i < cloud_data.points.size(); i++)
    {
        ...
        if (point_search_sqdis[0] < DISTANCE_SQ_THRESHOLD)
        {
            ...
            // search in the direction of increasing scan line   向上scan搜索  
            for (int j = closest_point_ind + 1; j < (int)cloud_scan.points.size(); j++)
            {
                // if not in nearby scans, end the loop
                if (int(cloud_scan.points[j].intensity) > (closest_point_scan_id + NEARBY_SCAN))
                ...
                float point_sqdis = sqrSum(cloud_scan.points[j].x - point_sel.x,
                                           cloud_scan.points[j].y - point_sel.y,
                                           cloud_scan.points[j].z - point_sel.z);
                // if in the same or lower scan line   当前scan以及以下  找min_point_sqdis2
                if (int(cloud_scan.points[j].intensity) <= closest_point_scan_id && point_sqdis < min_point_sqdis2)
                {
                    min_point_sqdis2 = point_sqdis;
                    min_point_ind2 = j;
                }
                // if in the higher scan line    当前帧之上的scan找min_point_sqdis3
...
/**
 *  @brief 与地图的线特征进行匹配  
 *  @details 与scan匹配的区别是，不用考虑scan中的线束了
 *  @param [out] features  匹配的结果   比如匹配的线特征的信息  
 */
template <typename PointType>
...
/** 
 *  @brief 在cloud_map中寻找point_ori的匹配线特征    没啥特别 ！！
 */
template <typename PointType>
...
/** 
 *  @brief 在cloud_map中寻找point_ori的匹配面特征    没啥特别 ！！
 */
template <typename PointType>
